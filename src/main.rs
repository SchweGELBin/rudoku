...
fn main() {
    ...
    // Solve grid
    let solved_grid = solve(grid.clone(), None).expect("Sudoku unsolvable");
    println!("Solved grid:");
...
fn generate() -> Grid<u8> {
    ...
    let mut grid = solve(start_grid.clone(), Some(seq.clone())).expect("Sudoku unsolvable");
    // Print solution
